...
- What happens to a memory that was previously marked `global` or `shared` when it's migrated into the new, all-private bundle model? It becomes private to its owning agent like every other memory — any cross-agent access it previously allowed is intentionally not preserved (see FR-014).
- What happens to an existing memory's content that already contains a bare `[[slug]]` link written under the old same-bundle convention? This is an accepted breaking change: migration does not rewrite it (see FR-014). Going forward, a bare `[[x]]` is parsed as a whole-bundle reference — it resolves correctly only if `x` happens to also be a bundle name, and becomes a broken link otherwise.
...
- **FR-003**: System MUST continue to support the memory graph and related-memory links an agent relies on to recall connected knowledge, including links that cross bundle boundaries (see FR-012), resolving them the same way regardless of the underlying storage representation.
- **FR-004**: A same-bundle (intra-bundle) concept reference MUST use a standard markdown relative-path link, `[label](path/to/concept.md)`, rather than wikilink syntax — keeping it natively clickable when the document is opened in a standard Markdown viewer outside Vizier (see Story 2, SC-011).
...
- **FR-009**: A developer or operator MUST be able to open any memory document directly, outside of Vizier's UI or API, and read its title, tags, and content, in order to understand what a given agent remembers.
...
- **FR-011**: System MUST tolerate a memory document being edited or removed directly on disk between Vizier operations, reflecting the change (or its absence) the next time the agent accesses that memory, rather than serving stale or corrupted data.
- **FR-012**: System MUST allow a concept document to link either to a specific concept in a different bundle owned by the same agent, using `[[bundle/slug]]`, or to another bundle as a whole (its index), using bare `[[bundle]]`, and MUST resolve both forms through the same related-memory/graph lookups used for intra-bundle links.
- **FR-013**: System MUST allow an agent's full set of memory bundles to be copied to a different Vizier workspace/deployment and load there with no data loss and no id/slug collisions with memories already present.
- **FR-014**: System MUST provide a migration path so memories already stored under the previous (row-based) mechanism are carried forward into the new bundle structure and remain accessible after the system adopts document-based storage; migrated memories become private to their owning agent (their previous `global`/`shared` visibility is not preserved) and land in that agent's default bundle unless already organized otherwise — no existing memory should become unreadable or vanish as a result of this change. This migration is structural/metadata-only: it does not rewrite embedded wikilink syntax inside existing memory content (see Edge Cases).
- **FR-015**: System MUST keep each memory's attachments as files within the same bundle, associated with their owning concept document, so that copying an agent's bundles (e.g., for migration) does not silently drop attachments.
- **FR-016**: System MUST automatically maintain each bundle's index document to reflect the current set of concept documents in that bundle, kept up to date as memories are written, updated, or deleted, so a developer or agent can see what's available without opening every concept document individually.
- **FR-017**: System MUST automatically maintain each bundle's log document as a chronological history of that bundle's memory writes and updates.
- **FR-018**: The WebUI memory graph MUST offer two levels of view: a bundle-level graph showing an agent's bundles as nodes and cross-bundle links as edges between them, and a concept-level graph — entered by opening a bundle — showing that bundle's concept documents as nodes and their links as edges, matching today's existing single-collection graph interaction.
- **FR-019**: The WebUI MUST let an operator export any one of an agent's bundles as a single downloadable `.zip` archive containing that bundle's concept documents, index document, log document, and attachments.
- **FR-020**: The WebUI MUST let an operator import a `.zip` archive as a bundle for a chosen agent, letting the operator pick the destination bundle name; if the destination bundle already exists, concept-level collisions MUST follow the same reject-not-overwrite rule as FR-010, with the operator informed of which concepts were skipped.
- **FR-021**: The agent MUST be able to learn the bundle model and its link conventions (same-bundle markdown links, cross-bundle `[[bundle/slug]]` and `[[bundle]]` wikilinks) through its existing discovery channels — the `BOOT.md` operating doctrine and the memory tools' name/description and field-level descriptions — both of which MUST be updated to teach the new conventions alongside the existing ones they already document.
...
- Reconciliation of manual, out-of-band edits to concept documents (Edge Cases, FR-011) happens lazily, on the next relevant read/query/write, rather than through continuous real-time filesystem watching.
- This feature drops the previous private/global/shared visibility distinction and the `shared_to` list entirely: every memory is private to its owning agent, with no cross-agent access of any kind. Existing memories marked `global` or `shared` are treated as private on migration (see FR-014) — any cross-agent access they previously allowed no longer applies after this change. Reintroducing cross-agent visibility, if ever needed, is a separate future feature.
...
- Cross-bundle links (FR-012) are scoped to bundles owned by the same agent; there is no mechanism in this feature for one agent's bundle to reference another agent's bundle.
- Adopting markdown-style links for same-bundle references and reserving `[[...]]` for cross-bundle references is a deliberate breaking change to the existing `[[slug]]` convention; it is accepted rather than preserved via a compatibility shim, consistent with this feature's general preference for a clean model over legacy-format compatibility (see FR-014 and Edge Cases).
